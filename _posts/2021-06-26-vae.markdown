...
For $p(x)$, the prior, we generally use a standard multivariate normal, i.e. $\mathcal{N}(\mathbb{0}, \mathcal{I})$.
...
The encoder network outputs the parameters of the multivariate distribution for $\mathbb{z}$ conditioned on $\mathbb{x}$, with the constraint that the covariance matrix is diagonal.
In other words, the encoder will output a vector of size $2n$, where $z \in \mathbb{R}^n$.
The first $n$ values of the vector are $\mathbb{\mu}$ and the last $n$ are $\sigma_1^2, \sigma_2^2, ..., \sigma_n^2$, i.e. $\mathrm{Diag}(\Sigma)$, giving us a multivariate normal distribution $\mathbb{z} \sim \mathcal{N}(\mathbb{\mu}, \Sigma)$.
Unfortunately, we would need to constrain our variance to be non-negative.
To solve this issue we instead output $\log \sigma_1, \log \sigma_2, ..., \log \sigma_n$, which are unconstrained for $\sigma \in (0, \inf)$.
