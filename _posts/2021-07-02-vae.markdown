...
&= \log p(x) + \min_{\theta, \phi} E_{Q}[\log \frac{q_\phi(z|x)}{p(z, x)}]
\end{aligned}
$$

where $\phi$ are the parameters of the encoder and $\theta$ are the parameters of the decoder.
$E_Q$ is the expectation w.r.t the measure defined by the distribution $q(z|x)$.
...
$$
\begin{aligned}
&\min_{\theta, \phi} E_{Q}[\log \frac{q_\phi(z|x)}{p(z, x)}] \\
&= \min_{\theta, \phi} E_{Q}[\log \frac{q_\phi(z|x)}{p_\theta(x|z) p(z)}] \\
&= \min_{\theta, \phi} - E_{Q}[\log p_\theta(x|z) + \log \frac{p(z)}{q_\phi(z|x)}] \\
&= \min_{\theta, \phi} - E_{Q}[\log p_\theta(x|z)] + D_{KL}(q_\phi(z|x)||p(z)) \\
&= \min_{\theta, \phi} - \mathcal{L_{\theta, \phi}}(Q)
\end{aligned}
$$

Where $\mathcal{L_{\theta, \phi}}(Q) = E_{Q}[\log p_\theta(x|z)] - D_{KL}(q_\phi(z|x)\|\|p(z))$ is the Evidence Lower Bound.
The naming for ELBO is justified as it is the lower bound of $\log p(x)$, the log evidence:

$$
\log p(x) = D_{KL}(q_\phi(z|x)||p(z|x)) + \mathcal{L_{\theta, \phi}}(Q)
$$

assuming $D_{KL}(q_\phi(z|x)||p(z|x)) \geq 0$, which follows from Gibb's inequality.
...
Now that we understand the components of the loss, we can see that there are three distributions that need to be determined to calculate the loss: the likelihood $p_\phi(x|z)$, the prior $p(x)$, and the variational posterior $q(z|x)$.
We've already alluded to the fact that the encoder network determines $q_\phi(z|x)$ and the decoder network determines $p_\theta(x|z)$.
For $p(x)$, the prior, we generally use a standard multivariate normal, i.e. $\mathcal{N}(\mathbf{0}, \mathcal{I})$.
With these components, it is possible to calculate the loss.

Let's examine how the encoder determines the distribution $q_\phi(z|x)$.
...
In other words, the encoder will output a vector of size $2n$, where $z \in \mathbb{R}^n$.
...
To solve this issue we instead have the encoder output the vector $(\log \sigma_1, \log \sigma_2, ..., \log \sigma_n)^T$, which are unconstrained for $\sigma \in (0, \infty)$.

If we want to sample $z$ from $\mathcal{N}(\mu, \Sigma)$, we have another problem.
How can we backpropagate through the sampling process?
The answer is using the reparametrization trick.
Instead of sampling from $\mathcal{N}(\mu, \Sigma)$, we sample instead $\epsilon$ from the standard normal $N(\mathbf{0}, \mathcal{I})$.
Then we perform the following operations to get $\mathbf{z}$:

$$
\mathbf{z} := \epsilon \odot \mathbf{\sigma} + \mathbf{\mu}
$$

where $\sigma = (\sigma_1, \sigma_2, ..., \sigma_n)^T$ can be obtained by exponentiating the outputs of the encoder, $(\log \sigma_1, \log \sigma_2, ..., \log \sigma_3)^T$, and $\epsilon \sim \mathcal{N}(0, \mathcal{I})$.
...
Without going into too much detail about how to calculate the $D_{KL}(q_\phi(z|x) || p(z))$, we can find that:

$$
D_{KL}(q_\phi(z|x) || p(z)) = \frac{1}{2} \sum_{i=1}^{n} - \log \sigma_i^2 - 1 + \sigma_i^2 + \mu_i
$$

Next we want to take a look at how we can obtain $p_\theta(x\|z)$ from the decoder network.
...
$$
-E_Q[\log p_\theta(x|z)] = \frac{1}{n} \sum_{i=1}^n (x_i - \hat{x}_i) ^ 2
$$
