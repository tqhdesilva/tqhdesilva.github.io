---
layout: post
title: "NanoFlow: Training Flow-Matching Models from Scratch on a Budget"
...
Before diving into the post, take a look at a training sample logged early in training, to illustrate some of the funny (and sometimes horrifying) model outputs:

<figure style="text-align: center;">
  <img src="/assets/images/nanoflow/derpy_dog.png" alt="Derpy Dog from early in training" width="420">
  <figcaption style="font-size: 0.9em; color: #666;">At least you can kind of tell it's a dog, right?</figcaption>
</figure>

Don't worry, it'll get better (slightly).

## Introduction

I'm sharing what I learned training conditional flow-matching image models from scratch.
All the core training and sampling here use ODE flow matching, although later we use the SDE interpretation of Flow matching during Flow-GRPO post-training.
...
- Layerwise scaling: make transformer layers wider in deeper layers. This pairs well with deferred masking, since the early layers are cheaper per token where we have the most tokens.
...
So it's more common to train an ImageNet model with hundreds to thousands of GPU-equivalent hours (not the ~64 H100 GPU hours we trained on).
...
We ended up going from training ~539 GFLOPs to ~41 EFLOPs on increasingly non-trivial datasets.
...
We also got some hands-on experience with post-training on image diffusion models.
Post-training was more complicated than pre-training, due to managing both rollouts and training, as well as many knobs to tune. There is also a lot of flexibility in terms of what rewards to optimize for.

This project felt very different from my industry experience training ML models. Typically spending even hundreds of hours of GPU time is not a big deal even for exploratory runs or evals, but running hobbyist experiments on your own dime changes a lot about how you think about compute.
I was a lot more conscious of estimating the E2E training cost before launching the run, and more pre-emptive about searching for ways to reduce the cost of the E2E training run.
